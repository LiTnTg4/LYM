...
local function GetFFlagFunc(flag)
    if type(flag) ~= "string" then 
        task.spawn(error, "string expected, got "..type(flag)) 
        return 
    end
    local FFlag = (UserInputService.TouchEnabled and 
        flag:gsub("DFInt", ""):gsub("DFFlag", ""):gsub("FFlag", ""):gsub("FInt", ""):gsub("DFString", ""):gsub("FString", "") 
        or flag)
    
    if getfflag(FFlag) ~= nil then
        return getfflag(FFlag)
    else
        task.spawn(error, "FFlag expected, got "..FFlag)
    end
end
...
local function ToggleFFlagFunc(flag, value)
    ...
    if getfflag(FFlag) ~= nil then
        ...
        local errorlog = getgenv().errorlog or "Bloxstrap/Logs/crashlog.txt"
...
Bloxstrap.ToggleFFlag = ToggleFFlagFunc
Bloxstrap.GetFFlag = GetFFlagFunc

-- ========== GuiLibrary.lua (RedZHub GUI Library) ==========
...
Bloxstrap.start = function(vis)
    ...
    if not vis then
        setidentity(8)
        local coreGui = game:GetService('CoreGui')
        if coreGui["redz Library V5"] then
            coreGui["redz Library V5"].Enabled = false
        end
    end
    ...
    FastFlags:AddTextBox({
        ...
        Description = "Paste JSON FFlags here",
        Callback = function(call)
            if call and call ~= "" then
                ...
                if success and type(fflags) == "table" then
    ...
    FastFlags:AddToggle({
        Name = "Gray sky",
        Description = "Turns the sky gray",
        Default = Bloxstrap.Config.GraySky,
        Flag = "GraySky",
        Callback = function(call)
            Bloxstrap.ToggleFFlag("FFlagDebugSkyGray", call)
        end
    })
    
    FastFlags:AddToggle({
        Name = "Desync",
        Description = "Lags your character behind",
        Default = Bloxstrap.Config.Desync,
        Flag = "Desync",
        Callback = function(call)
            Bloxstrap.ToggleFFlag("DFIntS2PhysicsSenderRate", call and 38000 or 15)
        end
    })
    
    -- 引擎设置选项卡
    EngineSettings:AddSection("Performance")
    EngineSettings:AddTextBox({
        Name = "Framerate limit",
        Description = "Set FPS limit (0 = unlimited)",
    ...
    EngineSettings:AddToggle({
        ...
        Flag = "DisablePlayerShadows",
        Callback = function(call)
            Bloxstrap.ToggleFFlag("FIntRenderShadowIntensity", call and 0 or 1)
        end
    })
    ...
    Appearance:AddToggle({
        ...
        Description = "Custom topbar style",
    ...
    Appearance:AddToggle({
        Name = "Spin Hotbar",
        Description = "Spins the Roblox logo",
    ...
    EngineSettings:AddToggle({
        ...
        Description = "Bring back the classic 'oof'",
