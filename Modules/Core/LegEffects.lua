...
local function hR6(c)
    ...
    local function h(p)
        if p and p.Transparency~=1 then
            p.Transparency=1
            for _,v in p:GetChildren()do if v:IsA"Attachment"then v.Visible=false end end
        end
    end
    h(u)h(l)h(o)
end
...
function LegEffects.enableR6(bool,player)
    ...
    if bool then
        if player.Character then cL() hR6(player.Character) end
    else
        if LegEffects.dLeg then LegEffects.dLeg:Destroy() LegEffects.dLeg=nil end
        if not getgenv().PhantomR15Leg then sR6(player.Character) end
    end
end
